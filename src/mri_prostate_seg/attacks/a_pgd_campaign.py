"""Adaptive-step PGD exactly as run for the reported campaign (Table I).

Provenance: copied verbatim from the `grade-a-refactor` worktree copy of
`mri_prostate_seg/attacks/a_pgd.py`, which is the code `run_adversarial_eval.py`
imported when the APGD row of Table I was produced. Kept as a separate module so
the reported results stay reproducible.

It differs from the current `a_pgd.py` in how the best iterate is tracked, and
the difference changes the attack's strength. Here `best_loss_val` starts at
`+inf`, so `loss_val > best_loss_val` never fires: the best iterate is never
updated, each checkpoint compares the current loss against the *initial* point's
loss, and the step size therefore almost always grows. `a_pgd.py` initializes to
`-inf`, so the comparison is against a running maximum, the halving branch fires
at nearly every checkpoint and the step size collapses. Measured on four fold-0
cases at eps = 0.1 (WG / TZ+CZ / PZ Dice): this module 0.518 / 0.337 / 0.144,
`a_pgd.py` 0.611 / 0.455 / 0.246, plain PGD 0.568 / 0.734 / 0.546. Only this
module reproduces the manuscript's FGSM < PGD < APGD ordering.

Do not "fix" the initialization here. Any change makes the module stop
reproducing the published numbers. New work should use `a_pgd.py`.
"""
...
def a_pgd_attack(
    ...
    checkpoint_steps = sorted(
        {max(1, int(f * n_steps)) for f in checkpoints if 0.0 < f < 1.0}
    )
    ...
    try:
        for k in range(1, n_steps + 1):
            ...
            x_adv = x_adv.detach() + current_step_size * step_dir.sign()
            ...
            if k in checkpoint_steps and k < n_steps:
                with torch.no_grad():
                    current_logits = model(x_adv)
                    current_loss = float(
                        loss_fn(current_logits, y, num_classes).detach().cpu()
                    )
                    prev_logits = model(best_x_adv)
                    prev_loss = float(
                        loss_fn(prev_logits, y, num_classes).detach().cpu()
                    )

                if current_loss > prev_loss * (1.0 + adaptation_rate):
