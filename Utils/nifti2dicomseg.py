...
def order_dcmfiles(series_dir: Path) -> List[Dataset]:
    ...
    **Future work: os.walk, DWI multi-frame, multiple series support.
    """
    ds = [dcmread(x) for x in _sitk_files_order(series_dir) if x.endswith(".dcm")]
...
def auto_seg_reader(seg_directory: Path) -> Dict[str, np.ndarray]:
    ...
    segments_dict = {
        key: value for key, value in segments_dict.items() if value["array"] is not None
    }
...
def clear_overlapping(seg_dict: Dict[str, dict]) -> dict:
    ...
    if "wg" in seg_dict:
        ...
        if "tz" in seg_dict:
            possible_overlap = True
            common_wg = common_wg & seg_dict["tz"]["array"]
    ...
    if "pz" in seg_dict:
        ...
        if possible_overlap:
            common_pz = common_pz.astype(bool)
...
def clean_zero_slices(seg_dict: Dict[str, dict]) -> dict:
    ...
    for key, value in seg_dict.items():
        nonzero_slices_indices = np.nonzero(np.any(value[check_key], axis=(1, 2)))[0]
...
def dim_organization_sequence(segment_ds: Dataset) -> Dataset:
    ...
    segment_ds.DimensionIndexSequence = Sequence([dim_idx_seq1, dim_idx_seq2])
...
def per_frame_group(
    reference_ds_list: List[Dataset], seg_dict: Dict[str, dict]
) -> Sequence:
    ...
    for value in seg_dict.values():
        if "seg_index" in value:
            for idx in value["seg_index"]:
                ...
                per_frame = frame_dataset(
                    value, idx, dataset, purpose_ds, derivation_code
                )
...
def nifti2dicomseg(seg_dir_path: Path, t2_path: Path, single_seg: str = ""):
    ...
    output = os.path.join("dicom_outputs", patient_id, study_uid, "segmentations")

    os.makedirs(output, exist_ok=True)

    if not single_seg:
        dcmwrite(
            os.path.join(output, "prostate_zones.dcm"),
            seg_ds,
            write_like_original=False,
        )
    else:
        dcmwrite(
            os.path.join(output, f"{single_seg}.dcm"), seg_ds, write_like_original=False
        )
